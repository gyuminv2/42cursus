...
namespace ft
{
    ...
    template <typename T>
    bool is_left(typename ft::rb_tree_node<T>::node_ptr node)
    {
    ...
    template <typename T>
    bool is_left(typename ft::rb_tree_node<T>::const_node_ptr node)
    {
        if (node && node->m_parent && node->m_parent->m_left)
            return node == node->m_parent->m_left;
        else
    ...
    template <typename T>
    typename ft::rb_tree_node<T>::node_ptr
    minimum(typename ft::rb_tree_node<T>::node_ptr root)
    {
    ...
    template <typename T>
    typename ft::rb_tree_node<T>::const_node_ptr
    minimum(typename ft::rb_tree_node<T>::const_node_ptr root)
    {
        while (root->m_left != NULL)
            root = root->m_left;
        return root;
    ...
    template <typename T>
    typename ft::rb_tree_node<T>::node_ptr
    maximum(typename ft::rb_tree_node<T>::node_ptr root)
    {
    ...
    template <typename T>
    typename ft::rb_tree_node<T>::const_node_ptr
    maximum(typename ft::rb_tree_node<T>::const_node_ptr root)
    {
        while (root->m_right != NULL)
            root = root->m_right;
        return root;
        ...
        public:
            ...
            rb_tree_iterator& operator++()
            {
                if (m_node->m_right != NULL)
                    m_node = minimum<value_type>(m_node->m_right);
                else
				{
                    while (!is_left<value_type>(m_node))
            ...
            rb_tree_iterator& operator--()
            {
                if (m_node->m_left != NULL)
                    m_node = maximum<value_type>(m_node->m_left);
                else
                {
                    while (is_left<value_type>(m_node))
                        m_node = m_node->m_parent;
    ...
    template<typename node_ptr, typename T>
    bool operator==(const rb_tree_iterator<node_ptr, T>& rhs, const rb_tree_iterator<node_ptr, T>& lhs) { return rhs.base() == lhs.base(); }
    template<typename node_ptr, typename T>
    bool operator!=(const rb_tree_iterator<node_ptr, T>& rhs, const rb_tree_iterator<node_ptr, T>& lhs) { return rhs.base() != lhs.base(); }
	...
	class rb_tree_const_iterator
    {
        ...
        public:
            ...
            const_node_ptr base() const { return m_node; }
    };

    template<typename const_node_ptr, typename T>
    bool operator==(const rb_tree_const_iterator<const_node_ptr, T>& rhs, const rb_tree_const_iterator<const_node_ptr, T>& lhs) { return rhs.base() == lhs.base(); }
    template<typename const_node_ptr, typename T>
    bool operator!=(const rb_tree_const_iterator<const_node_ptr, T>& rhs, const rb_tree_const_iterator<const_node_ptr, T>& lhs) { return rhs.base() != lhs.base(); }
    template<typename const_node_ptr, typename node_ptr, typename T>
    bool operator==(const rb_tree_const_iterator<const_node_ptr, T>& rhs, const rb_tree_iterator<node_ptr, T>& lhs) { return rhs.base() == lhs.base(); }
    template<typename const_node_ptr, typename node_ptr, typename T>
    bool operator!=(const rb_tree_const_iterator<const_node_ptr, T>& rhs, const rb_tree_iterator<node_ptr, T>& lhs) { return rhs.base() != lhs.base(); }
    template<typename node_ptr, typename const_node_ptr, typename T>
    bool operator==(const rb_tree_iterator<node_ptr, T>& rhs, const rb_tree_const_iterator<const_node_ptr, T>& lhs) { return rhs.base() == lhs.base(); }
    template<typename node_ptr, typename const_node_ptr, typename T>
    bool operator!=(const rb_tree_iterator<node_ptr, T>& rhs, const rb_tree_const_iterator<const_node_ptr, T>& lhs) { return rhs.base() != lhs.base(); }
        ...
        private :
            ...
            node_ptr replace_node(node_ptr node) const
            {
                if (node->m_left == NULL || node->m_right == NULL) //node의 자식이 0개 또는 1개일때
                    ...
                    return minimum<value_type>(node->m_right);     //node의 자식이 2개일 때
            }
            void erase(node_ptr root, node_ptr node)
            {
                ...
                if (is_left<value_type>(rep_node))   //rep_node가 parent의 왼쪽일때
                {
                ...
                if (rep_node != node)                           //node의 자식이 2개일때 (replace_node()함수의 else문 일때만) rep_node의 요소 저장
                {
                    ...
                    if (is_left<value_type>(node))   //node->parent에서 rep_node로 연결
                ...
                if (rep_color == black && root != NULL) //black + root, red + root, red + non-root 모두 해당되지 않음 (규칙 5에 어긋나지 않음) // case 1
                {
                    if (rep_child != NULL)          //자식을 black으로 바꿔서 black level 유지. (규칙 5)    // case 2
            ...
            void rebalance_for_erase(node_ptr root, node_ptr node, node_ptr sibling)
            {
                while (true)
                {
                    if (!is_left<value_type>(sibling))
                    {
                        if (get_node_color(sibling) == red) // case 3
                        {
                            sibling->color_change();
                            sibling->m_parent->color_change();
                            rotate_left(sibling->m_parent);
                            if (root == sibling->m_left)
                                root = sibling;
                            sibling = sibling->m_left->m_right;
                        }
                        if (get_node_color(sibling->m_left) == black && get_node_color(sibling->m_right) == black) // case 4
                        {
                            sibling->color_change();
                        ...
                        {
                            if (get_node_color(sibling->m_right) == black)  // case 5
                            {
                                sibling->m_left->color_change();
                                sibling->color_change();
                                rotate_right(sibling);
                                sibling = sibling->m_parent;
                            }
                            sibling->m_color = sibling->m_parent->m_color;  // case 6
                    ...
                    else
                    {
                        ...
                        {
                            if (get_node_color(sibling->m_left) == black)   // case 5
                            {
                                sibling->m_right->color_change();
                                sibling->color_change();
                                rotate_left(sibling);
                                sibling = sibling->m_parent;
                            }
            ...
            {
                while (node != root() && get_node_color(node->m_parent) == red) // case 1, 2
                {
                    node_ptr uncle = get_sibling(node->m_parent);
                    if (get_node_color(uncle) == red)   // case 3
                    {
                        uncle->color_change();
                        node = node->m_parent;
                        node->color_change();
                    ...
                    else if (is_left<value_type>(node->m_parent))   // case 4 right
                    {
                        if (!is_left<value_type>(node))
                        {
                            node = node->m_parent;
