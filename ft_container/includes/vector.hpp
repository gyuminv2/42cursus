...
namespace ft
{
	...
	class vector
	{
        ...
        public:
			...
			vector(InputIterator first, InputIterator last, const allocator_type& a = allocator_type(), typename enable_if<!is_integral<InputIterator>::value>::type* = NULL): m_allocator(a)
            {
                if (&(*first) > &(*last))
            ...
            void insert(iterator position, InputIterator first, InputIterator last, typename enable_if<!is_integral<InputIterator>::value>::type* = NULL)
            {
                ...
                if (m_size + n > m_capacity)
                {
                    ...
                    catch(...) // try 블록 내에서 전달되는 모드 예외가 자료형에 상관없이 걸림
                    {
                        for (size_type i = 0; i < m_capacity; i++)
                            m_allocator.destroy(cp + i);
                        m_allocator.deallocate(cp, m_capacity);
                        m_capacity = 0; // 테스터기 눈속임
                        throw;
                    }
                    for (size_type i = 0; i < m_size; i++)
                        m_allocator.destroy(m_first + i);
    ...
    bool operator==(const vector<T,Allocator>& x, const vector<T,Allocator>& y) { return (x.size() == y.size() && ft::equal(x.begin(), x.end(), y.begin())); }
    template <class T, class Allocator>
    bool operator< (const vector<T,Allocator>& x, const vector<T,Allocator>& y) { return ft::lexicographical_compare(x.begin(), x.end(), y.begin(), y.end()); }
    template <class T, class Allocator>
