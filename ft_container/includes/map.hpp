...
namespace ft
{
	...
	class map
    {
        ...
        public:
            ...
            T& at( const Key& key ) { return (*((this->insert(ft::make_pair(key, mapped_type()))).first)).second; }
            const T& at( const Key& key ) const { return (*((this->insert(ft::make_pair(key, mapped_type()))).first)).second; }
	};
	...
	template <class Key_, class T_, class Compare_, class Alloc>
	bool operator< (const map<Key_,T_,Compare_,Alloc>& x, const map<Key_,T_,Compare_,Alloc>& y) { return (ft::lexicographical_compare(x.begin(), x.end(), y.begin(), y.end())); }
	template <class Key_, class T_, class Compare_, class Alloc>
